...
int main(int argc, char *argv[])
{
    ...
    JThreadPool *p1 = j_thread_pool_new(thread_func, queue, 3, TRUE, NULL);
    JThreadPool *p2 = j_thread_pool_new(thread_func, queue, 2, TRUE, NULL);
    JThreadPool *p3 = j_thread_pool_new(thread_func, queue, 1, TRUE, NULL);
    if (p1 == NULL || p2 == NULL) {
        return -1;
    }
    jint i;
    for (i = 1; i < 6; i++) {
        if (!j_thread_pool_push(p1, JUINT_TO_JPOINTER(i), NULL) ||
            ...
            !j_thread_pool_push(p3, JUINT_TO_JPOINTER(8 - i), NULL)) {
            return -2;
        }
    }
